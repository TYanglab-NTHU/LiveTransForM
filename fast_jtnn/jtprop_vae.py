...
class JTPropVAE(nn.Module):
    ...
    def __init__(self, vocab, hidden_size, latent_size,prop_size,depthT, depthG):
        ...
        self.propNN = nn.Sequential(
                ...
                nn.Dropout(0.1),
                nn.Tanh(),
                nn.Linear(self.latent_size * 2,prop_size)
        ) # to output
        self.denticity_NN = nn.Sequential(
                # to first layer
                nn.Dropout(0.1),
                nn.Tanh(),
                nn.Linear(self.latent_size * 2,6)
        )        
        self.prop_loss = nn.L1Loss()
        self.denticity_loss = nn.CrossEntropyLoss(size_average=False)
        self.assm_loss = nn.CrossEntropyLoss(size_average=False)
    ...
    def forward(self, mol_batch, beta=0, wp=3.5, stats=False):
        # Extract input features
        x_batch, x_jtenc_holder, x_mpn_holder, x_jtmpn_holder, prop_batch = mol_batch
        
        # Encode molecular features
        x_tree_vecs, x_tree_mess, x_mol_vecs = self.encode(x_jtenc_holder, x_mpn_holder)
        
        # Sample from latent space
        z_mean, z_var = self.encode_latent(x_jtenc_holder, x_mpn_holder)
        ...
        # Extract property labels and split into matrices
        lfs_matrix, scs_matrix, denticity_matrix = create_var(prop_batch.transpose(0,1))
        # Predict properties and calculate losses
        denticity_pred = self.denticity_NN(z_mean)
        denticity_matrix = denticity_matrix.clone().long()
        denticity_loss = self.denticity_loss(denticity_pred.squeeze(0), (denticity_matrix - 1))
        prop_pred = self.propNN(z_mean)
        lfs_pred, scs_pred = prop_pred.transpose(0,1)
        scs_loss = self.prop_loss(scs_pred.squeeze(-1), scs_matrix)
        idxes_to_use = [False if torch.isnan(label) else True for label in lfs_matrix]
        if True in idxes_to_use:
            lfs_label = lfs_matrix[idxes_to_use]
            lfs_pred_filter = lfs_pred[idxes_to_use]
            lfs_loss = self.prop_loss(lfs_pred_filter, lfs_label)
            loss = word_loss + topo_loss + assm_loss + beta * kl_div + wp * lfs_loss + scs_loss + denticity_loss
            lfs_loss = lfs_loss.item()
        else:
            loss = word_loss + topo_loss + assm_loss + beta * kl_div + scs_loss + denticity_loss
            lfs_loss = float('nan')
        
        # Return results
        return loss, kl_div.item(), word_acc, topo_acc, assm_acc, lfs_loss, scs_loss.item(), denticity_loss.item()
