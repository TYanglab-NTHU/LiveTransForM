...
def enum_assemble(node, neighbors, prev_nodes=[], prev_amap=[]):
    ...
    for amap in all_attach_confs:
        ...
        cand_mol = Chem.MolFromSmiles(Chem.MolToSmiles(cand_mol), sanitize=False)
...
if __name__ == "__main__":
    ...
    def decode_test():
        ...
        for tot,s in enumerate(sys.stdin):
            ...
            cur_mol = Chem.MolFromSmiles(Chem.MolToSmiles(cur_mol, sanitize=False))
            set_atommap(cur_mol)
            dec_smiles = Chem.MolToSmiles(cur_mol)

            gold_smiles = Chem.MolToSmiles(Chem.MolFromSmiles(s, sanitize=False))
